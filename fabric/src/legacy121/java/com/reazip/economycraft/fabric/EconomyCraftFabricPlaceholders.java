...
final class EconomyCraftFabricPlaceholders {
    ...
    static void register() {
        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "balance"), (ctx, arg) -> {
        ...
        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "balance_formatted"), (ctx, arg) -> {
            if (!ctx.hasPlayer()) return PlaceholderResult.invalid("No player!");
            EconomyManager eco = EconomyCraft.getManager(ctx.server());
            return PlaceholderResult.value(PlaceholderValues.balanceFormatted(eco, ctx.player().getUUID()));
        });

        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "balance_short"), (ctx, arg) -> {
            if (!ctx.hasPlayer()) return PlaceholderResult.invalid("No player!");
            EconomyManager eco = EconomyCraft.getManager(ctx.server());
            return PlaceholderResult.value(PlaceholderValues.balanceShort(eco, ctx.player().getUUID()));
        });

        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "daily_sell_remaining"), (ctx, arg) -> {
            if (!ctx.hasPlayer()) return PlaceholderResult.invalid("No player!");
            EconomyManager eco = EconomyCraft.getManager(ctx.server());
            return PlaceholderResult.value(PlaceholderValues.dailySellRemaining(eco, ctx.player().getUUID()));
        });

        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "top_name"), (ctx, arg) -> {
        ...
        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "top_balance"), (ctx, arg) -> {
            String result = PlaceholderValues.topBalance(EconomyCraft.getManager(ctx.server()), arg);
            return result != null ? PlaceholderResult.value(result) : PlaceholderResult.invalid("No player!");
        });

        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "top_balance_formatted"), (ctx, arg) -> {
            String result = PlaceholderValues.topBalanceFormatted(EconomyCraft.getManager(ctx.server()), arg);
            return result != null ? PlaceholderResult.value(result) : PlaceholderResult.invalid("No player!");
        });

        Placeholders.register(ResourceLocation.fromNamespaceAndPath(EconomyCraft.MOD_ID, "top_balance_short"), (ctx, arg) -> {
