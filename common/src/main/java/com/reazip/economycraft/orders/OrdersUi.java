...
public final class OrdersUi {
            ...
            if (type == ClickType.PICKUP) {
                if (slot == 2) {
                    ...
                    if (current == null) {
                        ...
                        if (bal < cost) {
                            ((ServerPlayer) player).sendSystemMessage(Component.literal("Requester cannot afford this"));
                        } else {
    ...
    private static class ClaimMenu extends AbstractContainerMenu {
        private static class Delivery { ItemStack stack; boolean fromOrders; Delivery(ItemStack s, boolean o){stack=s;fromOrders=o;} }
        ...
        ClaimMenu(int id, Inventory inv, EconomyManager eco, UUID owner) {
            ...
            if (o != null) o.forEach(s -> items.add(new Delivery(s, true)));
            List<ItemStack> s = eco.getShop().claimDeliveries(owner);
            if (s != null) s.forEach(i -> items.add(new Delivery(i, false)));
            updatePage();
            for (int i = 0; i < 54; i++) {
                int r = i / 9;
                int c = i % 9;
        ...
        private void updatePage() {
            ...
            for (int i = 0; i < 45; i++) {
                ...
                container.setItem(i, items.get(idx).stack);
            }
            if (page > 0) {
                ItemStack prev = new ItemStack(Items.ARROW);
                prev.set(net.minecraft.core.component.DataComponents.CUSTOM_NAME, Component.literal("Previous page"));
            ...
            if (type == ClickType.PICKUP) {
                if (slot < 45) {
                    ...
                    if (global < items.size()) {
                        ...
                        super.clicked(slot, drag, type, player);
                        if (container.getItem(slot).isEmpty() && !before.isEmpty()) {
                            items.remove(global);
                            updatePage();
                        }
                        return;
        ...
        public void removed(Player player) {
            for (Delivery d : items) {
                if (d.fromOrders) eco.getOrders().addDelivery(owner, d.stack.copy());
                else eco.getShop().addDelivery(owner, d.stack.copy());
            }
            super.removed(player);
        }
    }
